<template>
    ...
    >
        <div class="block block-add-menu px-5 py-2" :class="{'block-mode-loading' : $store.state.UI.loader}">
			<h2 class="content-heading">Edit Transaksi Fivas</h2>
			<div class="row">
				<div class="col-sm-12">
                    <div class="row mb-2">
                    ...
                    <div class="row mb-2">
						...
						<div class="col-sm-6 mt-1">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" id="radio-prepaid" name="jenis_transaksi" :value="form.jenis_transaksi" checked
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" id="radio-postpaid" name="jenis_transaksi" :value="form.jenis_transaksi"
                                    @change="updateValueForm({
                                        name : 'jenis_transaksi',
                                        value : '1'
								    })"
                    ...
                    <div class="row mb-2">
						<div class="col-sm-12 text-end">
                            <button @click="$store.dispatch('TransaksifivasStoreEdit/toggleSidebar', false)" type="button" class="btn btn-danger">Batal</button>
                            <button @click="sendForm" type="button" class="btn btn-success">Update</button>
						</div>
					</div>
				</div>
			</div>
...
export default {
	...
	data() {
		return {
            errors: {},
            options: [{produk: [], harga: []},{produk: [], harga: []},{produk: [], harga: []},{produk: [], harga: []}],
		}
	},
    computed: {
        ...mapState('TransaksifivasStoreEdit', {
            form: state => state.form,
            visible_sidebar: state => state.visible_sidebar
        })
    },
    methods: {
        ...mapActions('TransaksifivasStoreEdit', {
            clearForm: 'clearForm',
        ...
        sendForm(){
			...
			let formData = {
                ...
                detail : this.form.detail.map((value, index) => {
                    return {
                        id: value?.id || null,
                        produk_id: value?.produk?.id || null,
			...
			Axios.put('transaksifivas/'+this.form.id, formData)
				.then(response => {
                    if(response.status === 200){
                        self.$store.dispatch('TransaksifivasStoreEdit/toggleSidebar', false);
                        self.$store.dispatch('TransaksifivasStoreIndex/getListTransaksiFivas');
                    }
				})
				.catch(error => {
